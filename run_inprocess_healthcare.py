"""In-process run of the Healthcare Concierge (notebook 8.4 pattern).

Bypasses A2A server. Middleware sees ALL nested events (ConditionalRequirement,
ChatModel, ThinkTool, HandoffTool, A2AAgent sub-calls).

Sub-agent servers must be running (policy/research/provider).
"""
...
from beeai_framework.adapters.gemini import GeminiChatModel  # noqa: F401
...
async def main() -> None:
    ...
    healthcare_agent = RequirementAgent(
        ...
        instructions=(
            ...
            Direct questions about Healthcare insurance policies to `{policy_agent.name}`
            Direct questions about Health Conditions to `{research_agent.name}`
            Direct questions about Healthcare providers to `{provider_agent.name}`

            IMPORTANT: When returning answers about healthcare providers, only give provider information from `{provider_agent.name}` and only provide insurance information based on the results from `{policy_agent.name}`.
